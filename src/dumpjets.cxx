    ...
    for ( eventIndex = 0; eventIndex < (Int_t) truthTree->GetEntries(); eventIndex++ ) {
        ...
        for(Int_t recoIndex = 0; recoIndex < (Int_t) recoTree->GetEntries(); recoIndex++) {
            ...
            foreach(TLorentzVector rjet, reco_jets) {
                if(atoi(argv[1]) == 0)myfile << "R" << "  " << "-" << " " << 3 << "  " << rjet.Pt() << " " << rjet.Eta() << "    " << rjet.Phi() << "    " << rjet.E() << "  " << 0 <<"\n";
                else myfile << EventNumber_reco << "    0   " << rjet.Pt() << " " << rjet.Eta() << "    " << rjet.Phi() << "    " << rjet.E() <<"\n";
            }

            foreach(TLorentzVector tjet, truth_jets) {
                if(atoi(argv[1]) == 0)myfile << "T3" << "  " << "-" << "    " << 2 << "  " << tjet.Pt() << " " << tjet.Eta() << "    " << tjet.Phi() << "    " << tjet.E() << "  " << 0 <<"\n";
                else myfile << EventNumber_truth << "    1   " << tjet.Pt() << " " << tjet.Eta() << "    " << tjet.Phi() << "    " << tjet.E() <<"\n";
            }
            myfile << "\n";
        }
